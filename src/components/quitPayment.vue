  ...
  <div class="payment_type" :style="selfStyle || {}">
    ...
    <div class="payInfo">
      ...
      <p class="pay_received">
        <span>本次退费</span>
        <span v-if="allReceivedFee === '无需退费'">{{allReceivedFee}}</span>
      ...
      <ul v-if="config.amount!== '待定' && config.amount !== 0">
        <li v-for="(pay, i) in config.payList">
          ...
          <span class="payment_input">
            <InputNumber v-model="pay.amount" :max="surplusMoneyList[i].surplusMoney" :min="0" :isFloat="true" :alignRight="true" :width="'200px'" :height="'38px'" @change="inputChange"></InputNumber><span style="margin-left: 5px;">元</span>
          </span>
        </li>
      </ul>
